...
Há ainda um limite superior para o tratamento. Trabalhos que comparam níveis crescentes de realce observam que o grupo de sobre-processamento fica abaixo do nível intermediário, o que indica um ponto de saturação a partir do qual a transformação passa a remover características que a rede aprenderia sozinha \cite{ozturk2018histopathological}. No domínio da inspeção elétrica, parte dos trabalhos concentra o esforço na arquitetura do detector em vez do tratamento prévio da imagem, como no modelo CSPD-YOLO, aplicado diretamente a imagens aéreas de fundo complexo \cite{liu2021}, e na rede convolucional com atenção recursiva RA-CNN, avaliada sobre imagens de isoladores em condições atmosféricas adversas \cite{wang2023}.
...
\begin{enumerate}
    \item \textbf{CPLID (\textit{Chinese Power Line Insulator Dataset}) \cite{tao2020detection}:} Base caracterizada por fundos homogêneos, com recortes de isoladores sadios e com fraturas visíveis nas peças de vidro ou cerâmica.
...
Devido ao fundo predominantemente homogêneo dessas imagens, as técnicas baseadas no realce direto da iluminação apresentaram os melhores resultados do conjunto. O uso isolado do filtro \textbf{\textit{Contrast Enhancement}} alcançou 95,35\% de acurácia, valor igualado por duas combinações híbridas, CLAHE seguido de \textit{Contrast Enhancement} e \textit{Contrast Enhancement} seguido de \textit{Equalize Histogram}.
...
Conversões como a de tons de cinza (\textit{Gray Scale}) não lidaram bem com as variações tonais e compartilham o menor valor observado, 63,33\% de acurácia, o que indica dependência da informação de cor para separar o componente do fundo. Em micrografias, por outro lado, a conversão para tons de cinza não degradou a acurácia \cite{chen2023robustness}. A divergência sugere que essa dependência decorre da complexidade do fundo da cena, e não constitui propriedade geral do classificador.
