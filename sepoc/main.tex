...
Esse cuidado reflete-se no desempenho das Redes Neurais Convolucionais (CNNs). Diferentemente das redes \textit{Feed-Forward}, as CNNs preservam a estrutura espacial da imagem por convoluções com \textit{kernels} locais, reduzem a dimensionalidade via \textit{Max-Pooling} e extraem mapas hierárquicos de textura e contraste \cite{LeCun2015}, conforme ilustrado na Fig. \ref{fig:cnn}. Sua capacidade de identificar anomalias depende, contudo, da representatividade dos dados de entrada.
...
Devido à menor poluição visual do fundo dessas imagens, as técnicas baseadas no realce direto da iluminação apresentaram os melhores resultados do conjunto. O uso isolado do filtro \textit{Contrast Enhancement} alcançou 95,35\% de acurácia e 96,55\% de \textit{F1-score}, e a acurácia foi igualada por duas combinações híbridas, CLAHE seguido de \textit{Contrast Enhancement} e \textit{Contrast Enhancement} seguido de \textit{Equalize Histogram}, esta última com 95,16\% de \textit{F1-score}.
...
O efeito no sentido oposto, esse sim, aparece com clareza. Filtros de desfoque, em especial o \textit{Gaussian Blur}, atenuaram as bordas das anomalias e levaram o desempenho a 67,44\%, 25,58 pontos abaixo da imagem original e abaixo do próprio piso majoritário. Em fundos de baixa poluição visual, portanto, o pré-processamento tem margem estreita para ajudar e ampla para prejudicar, e a utilidade prática da metodologia está em descartar as transformações destrutivas antes do treinamento. O desempenho das 14 variações testadas é apresentado na Fig. \ref{fig:cplid_acc}.
