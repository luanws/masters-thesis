...
Os trabalhos analisados demonstram a importância do pré-processamento na análise de imagens, mas também indicam que determinadas abordagens podem comprometer o desempenho da CNN. Em especial, \citeonline{rodrigues2020comparing} evidenciam que a eliminação de ruídos e artefatos pode não ser sempre benéfica. Além disso, \citeonline{Salvi2021} reforçam que técnicas de segmentação e normalização podem aprimorar a análise quando aplicadas corretamente. Cabe reconhecer que a dependência entre pré-processamento e desempenho já foi observada na literatura, em especial por \citeonline{ozturk2018histopathological} e \citeonline{rodrigues2020comparing}, ainda que em outros domínios de imagem. A contribuição desta dissertação é, portanto, de natureza incremental: sistematizar essa comparação e associá-la a um procedimento de otimização de parâmetros no domínio específico da inspeção de cadeias de isoladores, que não foi tratado de forma unificada pelos estudos analisados.

\section{Tabela Comparativa dos Trabalhos}

A tabela \ref{tab:comparacao_de_trabalhos_relacionados} compara os resultados de diferentes estudos sobre pré-processamento de imagens e seu impacto nos modelos de classificação de imagens. Os estudos variam desde melhorias no desempenho até riscos de sobre-processamento, destacando a falta de consenso quanto à escolha das técnicas. A metodologia proposta neste trabalho procura contribuir nesse ponto ao organizar, de forma sistemática, a comparação entre técnicas e a otimização de seus parâmetros para o domínio da inspeção de isoladores.
...
A análise desses estudos indica a ausência de uma abordagem unificada que combine a comparação sistemática de técnicas com a otimização de seus parâmetros no domínio da inspeção de isoladores. A proposta de um método para determinar os processamentos mais eficientes e otimizar seus parâmetros busca contribuir nesse sentido. Ao articular técnicas de pré-processamento e otimização de parâmetros, o trabalho pretende oferecer um procedimento prático e replicável, cujos princípios podem ser transpostos, com as devidas adaptações, a outros contextos de análise de imagens.

\section{Considerações finais do capítulo}

A metodologia apresentada neste trabalho distingue-se dos estudos revisados principalmente pela ênfase na etapa de pré-processamento. Enquanto boa parte dos trabalhos existentes concentra-se na construção de modelos de redes neurais e no uso pontual de técnicas tradicionais de pré-processamento e normalização, a metodologia deste trabalho organiza a comparação entre técnicas e associa a ela um procedimento de otimização de seus parâmetros. Trata-se de uma contribuição incremental, voltada à sistematização dessas etapas no domínio da inspeção de isoladores, cujos resultados podem servir de base para investigações futuras.