...
Esta seção trata do ponto de partida da metodologia, correspondente ao bloco ``Início'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), incluindo a inicialização do ambiente e a definição das métricas utilizadas para avaliar o desempenho ao longo do processo.
...
Esta seção detalha a etapa ``Seleção e validação do dataset'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), descrevendo a seleção, a validação e a preparação dos dados que servem de base para a metodologia. O fluxograma dessa etapa é apresentado na Figura \ref{fig:fluxograma_selecao_dataset} e cobre desde a definição das métricas até o balanceamento dos dados.
...
Esta seção detalha a etapa ``Pré-processamento'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), que abrange os processamentos de imagens voltados a otimizar os dados de entrada para melhorar o desempenho dos modelos de rede neural. O fluxograma da Figura \ref{fig:fluxograma_preprocessamento} ilustra as decisões e operações dessa fase.
...
% [ERRO PROPOSITAL - FACIL] Concordancia na voz passiva sintetica: "revisa-se os parametros" (deveria ser "revisam-se").
A dificuldade dessa etapa é baixa a média, pois são técnicas bem estabelecidas na literatura. Se o desempenho do modelo piorar após a aplicação dessas técnicas, revisa-se os parâmetros ou a ordem de aplicação em busca de um equilíbrio adequado.
...
Esta seção detalha a etapa ``Ajuste de parâmetros'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), dedicada à otimização automática dos parâmetros das técnicas de processamento de imagem. O fluxograma da Figura \ref{fig:fluxograma_ajuste_parametros} apresenta o fluxo de ajuste automático dos parâmetros.
...
Esta seção detalha a etapa ``Escolha e construção do modelo'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), que trata da seleção e da configuração das arquiteturas de rede neural utilizadas para avaliar a eficácia das técnicas de processamento de imagem. O fluxograma da Figura \ref{fig:fluxograma_escolha_modelo} ilustra as decisões envolvidas na escolha e na montagem do modelo.
...
% [ERRO PROPOSITAL - MEDIO] Funcao de ativacao incorreta: para classificacao multiclasse usa-se softmax, nao sigmoide (contradiz a secao de regras gerais da revisao bibliografica).
A configuração inclui a definição das camadas finais com um número de neurônios igual ao número de classes, a escolha de funções de ativação apropriadas (como a sigmoide, na classificação multiclasse) e a seleção de funções de perda adequadas (como a entropia cruzada). Também envolve a definição das métricas de avaliação relevantes, como acurácia, precisão, recall e F1-score.

Nesse tipo de modelo, a função de perda usualmente adotada é a entropia cruzada categórica, definida pela Equação~\ref{eq:perda_classificacao}:

% [ERRO PROPOSITAL - DIFICIL] Sinal negativo suprimido na entropia cruzada categorica; contradiz a Equacao \ref{eq:categorical_crossentropy} da revisao bibliografica, que traz o sinal correto.
\begin{equation}
    L_{\text{class}} = \frac{1}{N} \sum_{i=1}^{N} \sum_{c=1}^{C} y_{i,c} \log(\hat{y}_{i,c})
    \label{eq:perda_classificacao}
\end{equation}
...
Nesse tipo de modelo, a função de perda é composta, combinando a perda de localização das bounding boxes, a perda de confiança (presença ou ausência de objeto) e a perda de classificação, conforme a Equação~\ref{eq:perda_deteccao}:

\begin{equation}
    L_{\text{det}} = \lambda_{\text{loc}} L_{\text{loc}} + \lambda_{\text{conf}} L_{\text{conf}} + \lambda_{\text{cls}} L_{\text{cls}}
    \label{eq:perda_deteccao}
\end{equation}

em que $L_{\text{loc}}$, $L_{\text{conf}}$ e $L_{\text{cls}}$ são, respectivamente, as perdas de localização, de confiança e de classificação, e $\lambda_{\text{loc}}$, $\lambda_{\text{conf}}$ e $\lambda_{\text{cls}}$ são pesos que equilibram a contribuição de cada termo.
...
Nesse tipo de modelo, a função de perda mais comum é o erro médio quadrático (MSE), definido pela Equação~\ref{eq:perda_regressao}:

\begin{equation}
    L_{\text{reg}} = \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2
    \label{eq:perda_regressao}
\end{equation}
...
Esta seção detalha a etapa ``Treinamento'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), referente ao treinamento dos modelos de rede neural com os dados processados. O fluxograma da Figura \ref{fig:fluxograma_treinamento} apresenta as atividades dessa etapa.
...
Esta seção detalha a etapa ``Avaliação e ajustes'' do fluxo geral (Figura \ref{fig:fluxograma_geral}), que trata da avaliação dos resultados e dos mecanismos de ajuste que permitem a melhoria iterativa da metodologia, incluindo o ciclo de retroalimentação que retorna às etapas anteriores. O fluxograma da Figura \ref{fig:fluxograma_avaliacao} ilustra o ciclo de avaliação e ajustes.
