...
A influência do dataset na eficácia do processamento será outro aspecto investigado, considerando possíveis variações nos resultados devido ao uso de diferentes conjuntos de dados. Para isolar o efeito das técnicas de processamento, adota-se um modelo de rede neural de referência, mantido fixo ao longo dos experimentos; a comparação sistemática entre diferentes arquiteturas de rede é tratada como desdobramento futuro, e não como objeto de validação deste trabalho. Para aprimorar os processamentos de imagem, será desenvolvida uma metodologia que permita a combinação de diferentes processamentos unitários (processamentos de imagem que realizam uma única operação). Além disso, será criado um método de ajuste automático de parâmetros das técnicas de processamento de imagem, com o intuito de otimizar seus resultados sem exigir extensa intervenção manual.
...
\begin{figure}[h]
    \centering
    \caption{\label{fig:proposta}Diagrama da proposta de metodologia}
...
\begin{itemize}
    ...
    \item Adotar um modelo de rede neural de referência, mantido fixo, que permita avaliar de forma controlada e isolada o impacto das técnicas de processamento de imagem, deixando a comparação sistemática entre diferentes arquiteturas como desdobramento futuro.
